...
int determinaMaximul(int a, int b){
    if(a > b)
        return a;
    else
        return b;
...
int inaltime(Pagina * arbore){
    int h, i;
    if(!arbore)
        return 0;
    else{
        h = inaltime(arbore->p0);
        for(i = 1; i <= arbore->m; i++)
            h = determinaMaximul(h, 1 + inaltime(arbore->e[i].p));
        return h;
    }
}

void nrPaginilor(Pagina * arbore, int nivel, int *nr_pag){
    int i;
    if(!arbore)
        return;
    nrPaginilor(arbore->p0, nivel + 1, nr_pag);
    for(i = 1; i <= arbore->m; i++)
        nrPaginilor(arbore->e[i].p, nivel + 1, nr_pag);
    *nr_pag = *nr_pag + 1;
}
