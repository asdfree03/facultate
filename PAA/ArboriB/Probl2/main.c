...
int inaltime(Pagina *arbore){
    if(!arbore) return 0;
    else return 1+ inaltime(arbore->p0);
}
void pagini(Pagina *arbore){
    if (!arbore) return;
	pagini(arbore->p0);
...
int main()
{
	...
	for (i = 1; i <= n; i++){
        char c[30];
        scanf("%s",c);
		radacina = insereaza(radacina, c, NULL);
	}
	printf("Arborele-B:\n");
	afisare(radacina, 1);
	printf("Inaltimea arborelui e : %d\n",inaltime(radacina));
	printf("Nr de pagini ale arborelui  : %d\n",nr_pag(radacina));
	printf("\n\n");
}
